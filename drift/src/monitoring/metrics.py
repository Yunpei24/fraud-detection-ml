...
MULTIVARIATE_DRIFT_SCORE = Gauge(
    f"{NAMESPACE}_multivariate_drift_score",
    "Multivariate drift score from API"
)
...
def record_api_request(endpoint: str, method: str, status: str, duration: float) -> None:
    ...
    if status == "success":
        API_LAST_SUCCESS_TIMESTAMP.labels(endpoint=endpoint).set(datetime.utcnow().timestamp())
    
    logger.debug("api_request_recorded", endpoint=endpoint, method=method, status=status, duration=duration)
...
def update_drift_metrics(drift_results: Dict[str, Any]) -> None:
    ...
    try:
        ...
        OVERALL_DRIFT_DETECTED.labels(severity=severity_label).set(1 if overall_drift_detected else 0)
        ...
        logger.debug("drift_metrics_updated", overall_drift=overall_drift_detected, severity=severity_score)
