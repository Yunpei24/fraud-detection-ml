...
class Settings:
    ...
    database_url: str = os.getenv('DATABASE_URL', 'postgresql://fraud_user:password@localhost:5432/fraud_db')
    ...
    consecutive_drift_detections: int = int(os.getenv('CONSECUTIVE_DRIFT_DETECTIONS', '2'))

    # Alerting Configuration (essential)
    alert_email_enabled: bool = os.getenv('ALERT_EMAIL_ENABLED', 'true').lower() == 'true'
    alert_email_recipients: List[str] = field(default_factory=lambda: os.getenv('ALERT_EMAIL_RECIPIENTS', 'ml-team@example.com').split(','))
