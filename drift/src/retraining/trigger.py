...
class RetrainingTrigger:
    ...
    def should_retrain(
        self, drift_results: Dict[str, Any], force: bool = False
    ) -> tuple[bool, str]:
        """
        ...
        if severity_score >= 3:
            ...
            logger.warning(
                "retraining_triggered_critical_drift", severity_score=severity_score
            )
            return True, reason
        ...
        multivariate_drift_detected = drift_results.get("multivariate_drift", {}).get(
        ...
        if any(
            ...
            for sev in [
                data_drift_severity,
                target_drift_severity,
                concept_drift_severity,
            ]
        ):
        ...
        if (
            ...
            or data_drift_detected
            or data_drift_severity in ["HIGH", "CRITICAL"]
            or target_drift_detected
            or target_drift_severity in ["HIGH", "CRITICAL"]
    ...
    def get_retrain_priority(self, drift_results: Dict[str, Any]) -> str:
        """
        ...
        if concept_drift.get("drift_detected", False) and data_drift.get(
            "drift_detected", False
        ):
            return "HIGH"
    ...
    def trigger_airflow_dag(
        ...
        try:
            ...
            base_url = (
                getattr(self.settings, "airflow_webserver_url", None)
                or self.settings.airflow_api_url
            )
                ...
                conf["drift_summary"].update(
                    {
                        ...
                        "target_drift_detected": drift_results.get(
                            "target_drift", {}
                        ).get("drift_detected", False),
                        "concept_drift_detected": drift_results.get(
                            "concept_drift", {}
                        ).get("drift_detected", False),
                    }
                )
    ...
    def _check_cooldown(self) -> bool:
        ...
        cooldown_hours = getattr(
            ...
            getattr(
                self.settings, "retraining_cooldown_hours", 48
    ...
    def update_last_retrain_time(self) -> None:
        ...
        self.last_trigger_time = datetime.utcnow()
        logger.info(
            "last_retrain_time_updated", timestamp=self.last_trigger_time.isoformat()
        )
    ...
    def trigger_retraining(
        ...
        if success:
            ...
            logger.error(
                "retraining_trigger_failed", dag_id=dag_id, drift_results=drift_results
            )
...
def should_retrain(
    drift_results: Dict[str, Any], settings: Optional[Settings] = None
) -> bool:
    """
