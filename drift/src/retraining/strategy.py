...
def immediate_retrain(
    drift_results: Dict[str, Any], training_config: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
    """
...
def select_retraining_strategy(
    drift_results: Dict[str, Any], priority: str = "MEDIUM"
) -> str:
    """
    ...
    if priority == "CRITICAL" or concept_severity == "CRITICAL":
        logger.info(
            "strategy_selected", strategy="immediate", reason="critical_priority"
        )
        return "immediate"
    ...
    if priority == "HIGH":
        ...
        if drift_results.get("data_drift", {}).get(
            "drift_detected"
        ) and not drift_results.get("concept_drift", {}).get("drift_detected"):
            logger.info(
                "strategy_selected", strategy="incremental", reason="data_drift_only"
            )
            return "incremental"
        else:
            logger.info(
                "strategy_selected", strategy="immediate", reason="high_priority"
            )
            return "immediate"
