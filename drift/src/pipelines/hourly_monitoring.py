...
def fetch_recent_predictions(
    hours: int = 1, settings: Optional[Settings] = None
) -> pd.DataFrame:
    """
...
def check_thresholds(drift_results: Dict[str, Any]) -> bool:
    ...
    multivariate_drift = drift_results.get("multivariate_drift", {}).get(
...
def run_hourly_monitoring(settings: Optional[Settings] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        if should_retrain:
            ...
            logger.warning("triggering_retraining", priority=priority, reason=reason)
            ...
            if success:
                logger.info(
                    "retraining_triggered_successfully", dag_id="01_training_pipeline"
                )
            else:
