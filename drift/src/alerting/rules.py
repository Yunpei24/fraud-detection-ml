...
MODEL_DEGRADATION = {
    ...
    "condition": lambda recall_drop, fpr_increase: recall_drop > 0.05 or fpr_increase > 0.01,
    ...
    **kwargs
    ...
    if "thresholds" in rule and triggered:
        ...
        for level in ["critical", "high", "medium", "low"]:
            ...
            if threshold and any(v > threshold for v in kwargs.values() if isinstance(v, (int, float))):
...
def check_all_rules(
    ...
    if data_drift.get("drift_detected"):
        result = evaluate_alert_rule(
            "HIGH_DATA_DRIFT",
            drift_score=data_drift.get("avg_psi", 0)
        )
        if result["triggered"]:
    ...
    if target_drift.get("drift_detected"):
        result = evaluate_alert_rule(
            "HIGH_TARGET_DRIFT",
            relative_change=target_drift.get("relative_change", 0)
        )
        if result["triggered"]:
            alerts["triggered_rules"].append(result)
