...
class AzureSettings(BaseSettings):
    ...
    storage_account_name: str = Field(
        default="frauddetection", description="Storage account name"
    )
    storage_account_key: str = Field(
        default="devkey", description="Storage account key"
    )
...
class KafkaSettings(BaseSettings):
    ...
    bootstrap_servers: str = Field(
        default="localhost:9092", description="Bootstrap servers"
    )
    topic_transactions: str = Field(
        default="fraud-transactions", description="Transactions topic"
    )
    consumer_group: str = Field(
        default="fraud-detection-pipeline", description="Consumer group"
    )
...
class AlertSettings(BaseSettings):
    ...
    email_from: str = Field(
        default="fraud-detection@example.com", description="From email"
    )
...
class MLflowSettings(BaseSettings):
    ...
    experiment_name: str = Field(
        default="fraud-detection", description="Experiment name"
    )
...
class APISettings(BaseSettings):
    ...
    random_forest_model_name: str = Field(
        default="random_forest_fraud_model.pkl",
        description="Random Forest model filename",
    )
    nn_model_name: str = Field(
        default="nn_fraud_model.pth", description="Neural Network model filename"
    )
    isolation_forest_model_name: str = Field(
    ...
    shap_explainer_rf_name: str = Field(
        default="shap_explainer_rf.pkl", description="Random Forest SHAP explainer"
    )
    shap_explainer_nn_name: str = Field(
        default="shap_explainer_nn.pkl", description="Neural Network SHAP explainer"
    )
    shap_explainer_iforest_name: str = Field(
...
class DriftSettings(BaseSettings):
    ...
    target_drift_threshold: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Target drift threshold"
    )
    ...
    baseline_precision: float = Field(
        default=0.95, ge=0.0, le=1.0, description="Baseline precision"
    )
    baseline_fpr: float = Field(
        default=0.015, ge=0.0, le=1.0, description="Baseline FPR"
    )
    ...
    retraining_cooldown_hours: int = Field(
        default=48, description="Retraining cooldown"
    )
...
class TrainingSettings(BaseSettings):
    ...
    validation_split: float = Field(
        default=0.2, ge=0.0, le=1.0, description="Validation split"
    )
    ...
    feature_importance_threshold: float = Field(
        default=0.01, ge=0.0, le=1.0, description="Feature importance threshold"
    )
...
class AirflowSettings(BaseSettings):
    ...
    data_base_url: str = Field(
        default="http://fraud-data:8001", description="Data base URL"
    )
