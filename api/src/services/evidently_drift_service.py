...
class EvidentlyDriftService:
    ...
    def __init__(self, database_service: DatabaseService):
        ...
        self.column_mapping = ColumnMapping(
            ...
            categorical_features=[
                'transaction_type', 'customer_country', 'merchant_country',
                'currency', 'device_id', 'ip_address'
            ],
    ...
    async def detect_comprehensive_drift(self,
                                       window_hours: int = 24,
                                       reference_window_days: int = 30) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            reference_data = await self._get_reference_window_data(reference_window_days)
            ...
            results = {
                ...
                'target_drift': await self._detect_target_drift(current_data, reference_data),
                'concept_drift': await self._detect_concept_drift(current_data, reference_data),
                'multivariate_drift': await self._detect_multivariate_drift(current_data, reference_data),
                'drift_summary': {}
            }
    ...
    async def _detect_data_drift(self,
        ...
        try:
            ...
            report = Report(metrics=[
                ...
                ColumnDriftMetric(column_name='transaction_type', stattest='chisquare'),
                ...
                if metric['metric'] == 'ColumnDriftMetric':
                    ...
                    if result_data.get('drift_detected'):
                        data_drift['drifted_columns'].append({
                            ...
                            'stattest_name': result_data.get('stattest_name', 'unknown'),
                            'threshold': result_data.get('threshold', 0.05)  # Default threshold
                        })
    ...
    async def _detect_target_drift(self,
                                  current_data: pd.DataFrame,
                                  reference_data: pd.DataFrame) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            return {
                ...
                'rate_change_percent': ((current_fraud_rate - reference_fraud_rate) / reference_fraud_rate) * 100,
        ...
        try:
            ...
            for feature in key_features:
                if feature in current_data.columns and feature in reference_data.columns:
                    ...
                    reference_corr = reference_data[feature].corr(reference_data['is_fraud'])
            ...
            # Normalize the score
            concept_drift_score = concept_drift_score / len(key_features) if key_features else 0
    ...
    async def _detect_multivariate_drift(self,
                                        current_data: pd.DataFrame,
                                        reference_data: pd.DataFrame) -> Dict[str, Any]:
        """
            ...
            for test in results['tests']:
                ...
                if test['status'] == 'FAIL':
                    ...
                    if 'drifted_columns' in test['parameters']:
                        multivariate_drift['drift_columns_count'] = len(test['parameters']['drifted_columns'])
    ...
    async def run_sliding_window_analysis(self,
        ...
        try:
            ...
            for i in range(num_windows):
                window_start = datetime.utcnow() - timedelta(hours=total_hours - i * step_hours)
                ...
                reference_data = await self._get_reference_window_data(30)  # 30 days reference

                if not window_data.empty:
                    # Quick drift check for this window
                    window_drift = await self._quick_drift_check(window_data, reference_data)
    ...
    async def generate_drift_report(self,
        ...
        try:
            ...
            if analysis_results.get('data_drift', {}).get('dataset_drift_detected'):
                ...
                report['recommendations'].append('Consider retraining model with recent data')
            ...
            if analysis_results.get('target_drift', {}).get('drift_detected'):
                ...
                report['recommendations'].append('Immediate investigation required - fraud patterns may have changed')
            ...
            if analysis_results.get('concept_drift', {}).get('drift_detected'):
                ...
                report['recommendations'].append('Model may need recalibration or retraining')
    ...
    def _generate_drift_summary(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        drift_types = ['data_drift', 'target_drift', 'concept_drift', 'multivariate_drift']
        ...
        if summary['severity_score'] >= 3:
            summary['recommendations'].append('CRITICAL: Multiple drift types detected - immediate action required')
        elif summary['severity_score'] >= 2:
            summary['recommendations'].append('HIGH: Multiple drift patterns detected - investigate and consider retraining')
        elif summary['severity_score'] >= 1:
            summary['recommendations'].append('MEDIUM: Drift detected - monitor closely')
        else:
            summary['recommendations'].append('LOW: No significant drift detected - continue monitoring')
    ...
    async def _get_window_data(self, start_time: datetime, end_time: datetime) -> pd.DataFrame:
        """
    ...
    async def _quick_drift_check(self,
                                current_data: pd.DataFrame,
                                reference_data: pd.DataFrame) -> Dict[str, Any]:
        """
            ...
            if 'data_drift' in results and isinstance(results['data_drift'], dict):
                ...
                if 'dataset_drift_detected' in data_drift:
                    metrics_to_store.append({
                        ...
                        'threshold': 0.05,
                        'threshold_exceeded': data_drift.get('dataset_drift_detected', False),
                        'severity': 'HIGH' if data_drift.get('dataset_drift_detected') else 'LOW',
                        'feature_name': None,
                        'details': {'drifted_columns': data_drift.get('drifted_columns', [])}
                    })
                ...
                for column_info in data_drift.get('drifted_columns', []):
                    if isinstance(column_info, dict):
                        metrics_to_store.append({
                            ...
                            'metric_value': float(column_info.get('drift_score', 0)),
                            'threshold': 0.05,
                            'threshold_exceeded': column_info.get('drift_detected', False),
                            'severity': 'MEDIUM',
            ...
            if 'target_drift' in results and isinstance(results['target_drift'], dict):
                ...
                if 'drift_score' in target_drift:
                    metrics_to_store.append({
                        ...
                        'severity': 'CRITICAL' if target_drift.get('drift_detected') else 'LOW',
                        'feature_name': 'is_fraud',
                        'details': {
                            'current_fraud_rate': target_drift.get('current_fraud_rate'),
                            'reference_fraud_rate': target_drift.get('reference_fraud_rate'),
                            'rate_change_percent': target_drift.get('rate_change_percent')
                        }
                    })

            # Concept drift metrics
            if 'concept_drift' in results and isinstance(results['concept_drift'], dict):
                ...
                if 'drift_score' in concept_drift:
                    metrics_to_store.append({
                        ...
                        'severity': 'MEDIUM' if concept_drift.get('drift_detected') else 'LOW',
                        'feature_name': None,
                        'details': {
                            'stattest_name': concept_drift.get('stattest_name'),
                            'features_analyzed': concept_drift.get('features_analyzed', [])
                        }
                    })
