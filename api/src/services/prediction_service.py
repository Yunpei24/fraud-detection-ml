...
class TrafficRouter:
    ...
    def reload_config(self) -> None:
        ...
        self.config = self._load_config()
        self.logger.info(f"Traffic routing config reloaded: canary_enabled={self.config.canary_enabled}, canary_pct={self.config.canary_traffic_pct}%")
...
class PredictionService:
    ...
    async def predict_single(
        ...
        try:
            ...
            selected_model = self.canary_model if (use_canary and self.canary_model) else self.model
    ...
    def _prepare_features(self, features: List[float]) -> np.ndarray:
        ...
        if not hasattr(self.model, 'preprocessor') or not isinstance(self.model.preprocessor, DataPreprocessor):
    ...
    async def explain_prediction_shap(
        ...
        try:
            ...
            if "top_features" in explanation:
                for i, (feature_name, importance) in enumerate(explanation["top_features"].items()):
                    top_features.append({
            ...
            self.logger.info(f"Generated SHAP explanation for transaction {transaction_id}")
            return result
    ...
    async def get_feature_importance(self, model_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"Feature importance retrieval failed for {model_type}: {e}")
            raise PredictionFailedException(
                error_code="E802",
