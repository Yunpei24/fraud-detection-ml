...
async def comprehensive_drift_detection(
    ...
    try:
        ...
        logger.info(f"Comprehensive drift detection completed: drift_detected={result.get('drift_summary', {}).get('overall_drift_detected', False)}")
        return result
...
async def sliding_window_analysis(
    ...
    try:
        ...
        logger.info(f"Sliding window analysis completed: {len(result.get('windows', []))} windows analyzed")
        return result
...
async def generate_drift_report(
    ...
    try:
        ...
        logger.info(f"Drift report generated: severity={report.get('severity', 'UNKNOWN')}")
        return report
