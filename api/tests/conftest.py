...
from src.services import PredictionService, CacheService, DatabaseService, EvidentlyDriftService


@pytest.fixture
def client(mock_prediction_service, mock_cache_service, mock_database_service, mock_drift_service, mock_auth):
    """
    ...
    Returns:
        TestClient instance
    """
    from src.api.dependencies import get_prediction_service, get_cache_service, get_database_service, get_drift_service
...
def mock_prediction_service(mock_model):
    ...
    # Mock async methods
    service.predict_single = AsyncMock(return_value={
    ...
    service.predict_batch = AsyncMock(return_value={
        ...
        "predictions": [
            {
                ...
                "prediction": 0 if i % 5 != 0 else 1,  # Every 5th transaction is fraud
    ...
    service.get_model_info = Mock(return_value={"version": "test-v1.0", "models": ["rf", "xgb", "lgb"]})
...
def mock_drift_service():
    ...
    # Mock async methods
    service.detect_comprehensive_drift = AsyncMock(return_value={
    ...
    service.run_sliding_window_analysis = AsyncMock(return_value={
        'timestamp': '2025-01-15T10:30:00Z',
        'window_size': '24h',
        'step_size': '6h',
        'analysis_period': '7d',
    ...
    service.generate_drift_report = AsyncMock(return_value={
        'timestamp': '2025-01-15T10:30:00Z',
        'summary': {
            'overall_drift_detected': False,
            'severity_score': 0
