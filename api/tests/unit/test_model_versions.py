...
class TestModelMetrics:
    def test_metrics_to_dict(self):
        metrics = ModelMetrics(
            accuracy=0.95, precision=0.96, recall=0.94, f1_score=0.95, auc_roc=0.99
        )
...
class TestModelRegistry:
    ...
    def test_get_current_version(self, registry):
        metrics = ModelMetrics(0.95, 0.96, 0.94, 0.95, 0.99)
        version = registry.register_model(
            version_id="xgb-v1.0.0", framework=ModelFramework.XGBOOST, metrics=metrics
        )
    ...
    def test_get_best_model(self, registry):
        ...
        registry.register_model(
            version_id="xgb-v1.0.0", framework=ModelFramework.XGBOOST, metrics=metrics1
        )
        registry.register_model(
    ...
    async def test_transition_stage(self, registry):
        ...
        result = await registry.transition_stage(
            version_id="xgb-v1.0.0", new_stage=ModelStage.PRODUCTION
        )
    ...
    async def test_transition_nonexistent_version(self, registry):
        result = await registry.transition_stage(
            version_id="nonexistent", new_stage=ModelStage.PRODUCTION
        )
