...
class TestEvidentlyDriftService:
    ...
    def sample_reference_data(self):
        ...
        data = {
            ...
            'amount': np.random.normal(95, 15, n_samples),  # Slightly different distribution
            'is_fraud': np.random.choice([0, 1], n_samples, p=[0.97, 0.03]),  # Lower fraud rate
            ...
            'transaction_type': np.random.choice(['online', 'pos', 'atm'], n_samples, p=[0.6, 0.3, 0.1]),
            'customer_country': np.random.choice(['US', 'UK', 'DE', 'FR'], n_samples, p=[0.5, 0.3, 0.1, 0.1]),
            'merchant_country': np.random.choice(['US', 'UK', 'DE', 'FR'], n_samples, p=[0.4, 0.4, 0.1, 0.1]),
            'currency': np.random.choice(['USD', 'EUR', 'GBP'], n_samples, p=[0.6, 0.3, 0.1]),
            'device_id': [f'DEV_{i%200:03d}' for i in range(n_samples)],
            'ip_address': [f'10.0.0.{i%255}' for i in range(n_samples)],
            'time': [datetime.utcnow() - timedelta(days=i) for i in range(30, 30 + n_samples)]
        }
    ...
    @pytest.mark.asyncio
    async def test_detect_data_drift_success(self, drift_service, sample_current_data, sample_reference_data):
        """Test successful data drift detection."""
        result = await drift_service._detect_data_drift(sample_current_data, sample_reference_data)
    ...
    @pytest.mark.asyncio
    async def test_detect_target_drift_success(self, drift_service, sample_current_data, sample_reference_data):
        """Test successful target drift detection."""
        result = await drift_service._detect_target_drift(sample_current_data, sample_reference_data)
    ...
    @pytest.mark.asyncio
    async def test_detect_concept_drift_success(self, drift_service, sample_current_data, sample_reference_data):
        """Test successful concept drift detection."""
        result = await drift_service._detect_concept_drift(sample_current_data, sample_reference_data)
    ...
    @pytest.mark.asyncio
    async def test_detect_multivariate_drift_success(self, drift_service, sample_current_data, sample_reference_data):
        """Test successful multivariate drift detection."""
        result = await drift_service._detect_multivariate_drift(sample_current_data, sample_reference_data)
    ...
    @pytest.mark.asyncio
    async def test_comprehensive_drift_insufficient_data(self, drift_service, mock_db_service):
        """Test comprehensive drift detection with insufficient data."""
        # Mock empty dataframes
        mock_db_service.fetch_all.return_value = []

        result = await drift_service.detect_comprehensive_drift(window_hours=24, reference_window_days=30)

        assert 'error' in result
        assert 'Insufficient data' in result['error']

    @pytest.mark.asyncio
    async def test_comprehensive_drift_success(self, drift_service, mock_db_service, sample_current_data, sample_reference_data):
    ...
    @pytest.mark.asyncio
    async def test_sliding_window_analysis_success(self, drift_service, mock_db_service, sample_current_data, sample_reference_data):
    ...
    @pytest.mark.asyncio
    async def test_generate_drift_report_no_alerts(self, drift_service, mock_db_service):
    ...
    @pytest.mark.asyncio
    async def test_generate_drift_report_with_alerts(self, drift_service, mock_db_service):
    ...
    @pytest.mark.asyncio
    async def test_error_handling_in_comprehensive_drift(self, drift_service, mock_db_service):
    ...
    @pytest.mark.asyncio
    async def test_quick_drift_check(self, drift_service, sample_current_data, sample_reference_data):
        """Test quick drift check for sliding windows."""
        result = await drift_service._quick_drift_check(sample_current_data, sample_reference_data)
