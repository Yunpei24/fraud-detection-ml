...
class TestDriftAPIIntegration:
    ...
    def test_comprehensive_drift_detection_insufficient_data(
        self, client, mock_drift_service
    ):
        """Test comprehensive drift detection with insufficient data."""
        mock_drift_service.detect_comprehensive_drift.side_effect = Exception(
    ...
    def test_sliding_window_analysis_success(self, client, mock_database_service):
        ...
        with patch(
            "src.services.evidently_drift_service.EvidentlyDriftService",
            return_value=mock_drift_service,
        ):
            response = client.post(
    ...
    def test_generate_drift_report_no_alerts(self, client, mock_database_service):
        ...
        with patch(
            ...
            response = client.post(
                "/api/v1/drift/generate-report", json=analysis_results
            )
    ...
    def test_generate_drift_report_with_alerts(self, client, mock_drift_service):
        ...
        mock_drift_service.generate_drift_report.return_value = {
            "timestamp": "2025-01-15T10:30:00Z",
            "summary": {"overall_drift_detected": True, "severity_score": 3},
            "recommendations": [
                "CRITICAL: Multiple drift types detected - immediate action required",
        ...
        response = client.post("/api/v1/drift/generate-report", json=analysis_results)
    ...
    def test_invalid_parameters(self, client):
        ...
        assert response.status_code in [
            200,
            500,
        ]  # Either success or service-level validation error
