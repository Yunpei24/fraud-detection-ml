        ...
        try:
            ...
            for model_uri in model_uris:
                ...
                try:
                    ...
                    client = MlflowClient()
                    model_details = client.get_model_version_by_alias(model_name, "Staging")
                    run_id = model_details.run_id
                    
                    # Download SHAP explainer artifact
                    explainer_path = client.download_artifacts(run_id, f"shap_explainer_{model_name.replace('fraud_detection_', '')}.pkl")
            ...
            logger.info("💾 Saving models and SHAP explainers to Azure File Share canary directory...")
            ...
            for model_name, model in loaded_models.items():
                ...
                short_name = model_name.replace("fraud_detection_", "")  # e.g., "xgboost"
                ...
                if model_name in loaded_explainers:
                    explainer_path = canary_models_dir / f"shap_explainer_{short_name}.pkl"
                    joblib.dump(loaded_explainers[model_name], explainer_path)
                    logger.info(f"   Saved SHAP explainer for {model_name} to {explainer_path}")
            ...
            logger.info(f" Traffic routing updated: {traffic_pct}% → canary models, {100-traffic_pct}% → champion models")
            
            # 4. Validate deployment
            logger.info(" Validating ensemble deployment...")
            # In production, this would test the ensemble prediction
            
            logger.info(f" Ensemble canary deployment complete: {traffic_pct}% traffic to challenger ensemble")
            return True
