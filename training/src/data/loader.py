...
def load_local_creditcard(path: str | Path = "data/raw/creditcard.csv", validate: bool = True) -> pd.DataFrame:
    """
    Legacy function for backward compatibility.
    Prefer using load_training_data() for production.
    """
    logger.warning("load_local_creditcard() is deprecated. Use load_training_data() instead.")
    return load_local_csv(path, validate=validate)


def load_reference_data(window_days: int = 30, conn_str: Optional[str] = None) -> pd.DataFrame:
    """
