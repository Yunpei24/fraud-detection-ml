...
class IsolationForestModel:
    ...
    def __init__(
        ...
        logger.info(
            f"IsolationForestModel initialized (contamination={contamination}, tune_hyperparams={tune_hyperparams})"
        )
    ...
    def fit(
        ...
        if hasattr(self.model, "offset_"):
            logger.info(
                f"Isolation Forest training completed (offset={self.model.offset_:.4f})"
            )
        else:
    ...
    def predict_proba(
        ...
        fraud_proba = 1 / (
            1 + np.exp((scores - threshold) / np.abs(np.std(scores) + 1e-8))
        )
