...
def compare_ensembles(
    ...
    def _evaluate_ensemble(ensemble_dict: Dict[str, Any], name: str):
        ...
        logger.info(
            f"   {name}: AUC={metrics.get('auc', 0):.4f}, "
...
def measure_ensemble_latency(
    ensemble_models: Dict[str, Any], X: np.ndarray, n_trials: int = 10
) -> float:
    """
...
def statistical_test(
    ...
    *,
    ...
    logger.info(
        f"Statistical test: {primary_metric} improvement = {improvement_pct:.2f}%"
    )
...
def decide_deployment(
    ...
    *,
    ...
    logger.info(
        f"   Recall: {chal_recall:.4f} ({'✅' if meets_recall else '❌'} >= {min_recall})"
    )
    logger.info(f"   FPR: {chal_fpr:.4f} ({'✅' if meets_fpr else '❌'} <= {max_fpr})")
    logger.info(
        f"   Latency: {challenger_latency:.2f}ms ({'✅' if meets_latency else '❌'} <= {max_latency_ms}ms)"
    )
    ...
    if is_better:
        reason = (
            f"Challenger improved F1 by >={min_improvement_pct}% and meets constraints"
        )
...
def run_comparison(
    ...
    **kwargs,
    ...
    comparison_results = compare_ensembles(
        champion_models, challenger_models, X_test, y_test
    )
    ...
    result = decide_deployment(
        comparison_results, champion_latency, challenger_latency, **kwargs
    )
