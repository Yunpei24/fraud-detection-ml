...
from src.pipelines.realtime_pipeline import RealtimePipeline, KAFKA_AVAILABLE
...
class TestRealtimePipeline:
    ...
    @patch('src.pipelines.realtime_pipeline.KAFKA_AVAILABLE', True)
    @patch('src.pipelines.realtime_pipeline.KafkaConsumer')
    def test_load_from_kafka_batch(self, mock_kafka_consumer, sample_transactions_batch):
    ...
    def test_clean_and_preprocess(self, mock_db_service, sample_transactions_df):
        ...
        dirty_df.loc[1, 'transaction_id'] = dirty_df.loc[0, 'transaction_id']  # Duplicate
    ...
    def test_clean_and_preprocess_empty_df(self):
        ...
        columns = ['Time', 'amount'] + [f'V{i}' for i in range(1, 29)] + ['transaction_id', 'timestamp']
    ...
    def test_predict_batch_success(self, mock_requests_post, sample_clean_transactions_df):
    ...
    @patch('src.pipelines.realtime_pipeline.requests.post')
    def test_predict_batch_api_error(self, mock_requests_post, sample_clean_transactions_df):
    ...
    def test_send_to_webapp_success(self, mock_requests_post, sample_fraud_predictions):
        ...
        pipeline = RealtimePipeline(webapp_url='http://localhost:3000', connect_db=False)
    ...
    def test_execute_batch_success(self, mock_send_webapp, mock_save_db, mock_predict, 
                                   mock_clean, mock_load_kafka, sample_clean_transactions_df, 
    ...
    @patch.object(RealtimePipeline, 'load_from_kafka_batch')
    @patch.object(RealtimePipeline, 'clean_and_preprocess')
    def test_execute_batch_all_filtered(self, mock_clean, mock_load_kafka, sample_transactions_df):
    ...
    @patch.object(RealtimePipeline, 'clean_and_preprocess')
    @patch.object(RealtimePipeline, 'predict_batch')
    def test_execute_batch_prediction_error(self, mock_predict, mock_clean, mock_load_kafka,
    ...
    def test_process_stream_buffer(self, sample_transactions_list):
        ...
        with patch.object(RealtimePipeline, 'clean_and_preprocess') as mock_clean, \
             patch.object(RealtimePipeline, 'predict_batch') as mock_predict, \
             patch.object(RealtimePipeline, 'save_to_database') as mock_save, \
             patch.object(RealtimePipeline, 'send_to_webapp') as mock_send:
    ...
    def test_batch_execution_timing(self):
        ...
        with patch.object(RealtimePipeline, 'load_from_kafka_batch') as mock_load, \
             patch.object(RealtimePipeline, 'clean_and_preprocess') as mock_clean, \
             patch.object(RealtimePipeline, 'predict_batch') as mock_predict, \
             patch.object(RealtimePipeline, 'save_to_database') as mock_save, \
             patch.object(RealtimePipeline, 'send_to_webapp') as mock_send:
            ...
            mock_predict.return_value = pd.DataFrame({'test': [1, 2, 3], 'predicted_fraud': [0, 1, 0]})
    ...
    def test_fraud_alerts_only_sent_to_webapp(self, sample_mixed_predictions_df):
        ...
        fraud_only = sample_mixed_predictions_df[sample_mixed_predictions_df['predicted_fraud'] == 1]
        ...
        with patch.object(RealtimePipeline, 'load_from_kafka_batch') as mock_load, \
            ...
            mock_load.return_value = sample_mixed_predictions_df.drop(['predicted_fraud', 'fraud_probability'], axis=1)
            mock_clean.return_value = sample_mixed_predictions_df.drop(['predicted_fraud', 'fraud_probability'], axis=1)
...
def sample_transaction():
    ...
    return {
        ...
        'amount': 150.75,
        'Class': 0,
        'V1': -1.23, 'V2': 0.45, 'V3': -0.67,
        'V4': 1.23, 'V5': -0.89, 'V6': 0.12,
        'V7': -0.34, 'V8': 0.56, 'V9': -0.78,
        'V10': 0.90, 'V11': -0.4, 'V12': 0.9,
        'V13': -0.6, 'V14': 0.3, 'V15': -0.2,
        'V16': 0.7, 'V17': -0.5, 'V18': 0.1,
        'V19': -0.8, 'V20': 0.4, 'V21': -0.3,
        'V22': 0.6, 'V23': -0.9, 'V24': 0.2,
        'V25': -0.1, 'V26': 0.5, 'V27': -0.7,
        'V28': 0.3,
...
def sample_transactions_batch():
    ...
    return [
        {
            'Time': 100.0, 'amount': 100.0, 'Class': 0,
            'V1': -1.0, 'V2': 0.5, 'V3': -0.5,
            'V4': 1.0, 'V5': -0.8, 'V6': 0.2,
            'V7': -0.3, 'V8': 0.6, 'V9': -0.7,
            'V10': 0.8, 'V11': -0.4, 'V12': 0.9,
            'V13': -0.6, 'V14': 0.3, 'V15': -0.2,
            'V16': 0.7, 'V17': -0.5, 'V18': 0.1,
            'V19': -0.8, 'V20': 0.4, 'V21': -0.3,
            'V22': 0.6, 'V23': -0.9, 'V24': 0.2,
            'V25': -0.1, 'V26': 0.5, 'V27': -0.7,
            'V28': 0.3,
            'transaction_id': 'txn_001', 'timestamp': '2025-01-15T10:00:00Z'
        },
        {
            'Time': 200.0, 'amount': 200.0, 'Class': 1,
            'V1': -2.0, 'V2': 1.0, 'V3': -1.0,
            'V4': 2.0, 'V5': -1.6, 'V6': 0.4,
            'V7': -0.6, 'V8': 1.2, 'V9': -1.4,
            'V10': 1.6, 'V11': -0.8, 'V12': 1.3,
            'V13': -1.2, 'V14': 0.7, 'V15': -0.5,
            'V16': 1.1, 'V17': -0.9, 'V18': 0.6,
            'V19': -1.5, 'V20': 0.8, 'V21': -0.7,
            'V22': 1.0, 'V23': -1.3, 'V24': 0.4,
            'V25': -0.6, 'V26': 0.9, 'V27': -1.1,
            'V28': 0.5,
            'transaction_id': 'txn_002', 'timestamp': '2025-01-15T11:00:00Z'
        },
        {
            'Time': 300.0, 'amount': 300.0, 'Class': 0,
            'V1': -0.5, 'V2': 0.25, 'V3': -0.25,
            'V4': 0.5, 'V5': -0.4, 'V6': 0.1,
            'V7': -0.15, 'V8': 0.3, 'V9': -0.35,
            'V10': 0.4, 'V11': -0.2, 'V12': 0.45,
            'V13': -0.3, 'V14': 0.15, 'V15': -0.1,
            'V16': 0.35, 'V17': -0.25, 'V18': 0.05,
            'V19': -0.4, 'V20': 0.2, 'V21': -0.15,
            'V22': 0.3, 'V23': -0.45, 'V24': 0.1,
            'V25': -0.05, 'V26': 0.25, 'V27': -0.35,
            'V28': 0.15,
            'transaction_id': 'txn_003', 'timestamp': '2025-01-15T12:00:00Z'
        }
    ]
...
def sample_clean_transactions_df():
    ...
    return pd.DataFrame({
        'Time': [100.0, 200.0],
        'amount': [150.0, 250.0],
        'V1': [-1.0, -2.0], 'V2': [0.5, 1.0], 'V3': [-0.5, -1.0],
        'V4': [1.0, 2.0], 'V5': [-0.8, -1.6], 'V6': [0.2, 0.4],
        'V7': [-0.3, -0.6], 'V8': [0.6, 1.2], 'V9': [-0.7, -1.4],
        'V10': [0.8, 1.6], 'V11': [-0.4, -0.8], 'V12': [0.9, 1.3],
        'V13': [-0.6, -1.2], 'V14': [0.3, 0.7], 'V15': [-0.2, -0.5],
        'V16': [0.7, 1.1], 'V17': [-0.5, -0.9], 'V18': [0.1, 0.6],
        'V19': [-0.8, -1.5], 'V20': [0.4, 0.8], 'V21': [-0.3, -0.7],
        'V22': [0.6, 1.0], 'V23': [-0.9, -1.3], 'V24': [0.2, 0.4],
        'V25': [-0.1, -0.6], 'V26': [0.5, 0.9], 'V27': [-0.7, -1.1],
        'V28': [0.3, 0.5],
