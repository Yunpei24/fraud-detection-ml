...
class TestQualityValidator:
    ...
    def test_check_duplicates_with_duplicates(self):
        ...
        df = pd.DataFrame(
            {"col1": [1, 2, 3, 1, 2], "col2": ["a", "b", "c", "a", "b"]}  # Duplicates
        )
        ...
        )  # 4 duplicate rows (all instances of duplicates)
        ...
        df = pd.DataFrame(
            {
                "values": [
                    ...
                    5,
                    100,
                ]  # 100 is extreme outlier with lower threshold
            }
        )
        ...
        df = pd.DataFrame(
            {
                ...
                "outlier_col": [
                    ...
                    4,
                    1000,
                ],  # 1000 is outlier with lower threshold
            }
        )

        result = validator.check_outliers(
            df, numeric_columns=["outlier_col"], std_threshold=1.5
        )
    ...
    def test_check_data_types_invalid(self):
        ...
        expected_types = {"int_col": "int64", "float_col": "float64"}
    ...
    def test_check_data_types_missing_column(self):
        ...
        expected_types = {"existing_col": "int64", "missing_col": "float64"}
    ...
    def test_validate_batch_comprehensive(self):
        ...
        expected_types = {"id": "int64", "amount": "float64", "category": "object"}
    ...
    def test_data_type_validation_edge_cases(self):
        ...
        df = pd.DataFrame(
            {"int_with_nulls": [1, 2, None], "float_with_nulls": [1.1, 2.2, None]}
        )

        expected_types = {"int_with_nulls": "int64", "float_with_nulls": "float64"}
    ...
    def test_logging_on_issues(self, mock_logger):
        ...
        df = pd.DataFrame({"col": [1, np.nan, 3, np.nan]})  # Missing values
