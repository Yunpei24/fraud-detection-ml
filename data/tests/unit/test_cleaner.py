...
class TestDataCleaner:
    ...
    def test_remove_duplicates_no_subset(self, sample_dataframe_with_duplicates):
        ...
        with patch.object(cleaner, 'remove_duplicates', wraps=cleaner.remove_duplicates) as mock_remove:
    ...
    def test_remove_duplicates_keep_last(self, sample_dataframe_with_duplicates):
        ...
        df_with_dups = pd.DataFrame({
            ...
            'value': [15, 20, 15, 15]  # Alice appears three times, last one has value 15
        })
    ...
    def test_handle_missing_values_mean_strategy(self):
        ...
        result_df = cleaner.handle_missing_values(df, numeric_strategy='mean', categorical_strategy='mode')
    ...
    def test_handle_missing_values_drop_strategy(self):
        ...
        result_df = cleaner.handle_missing_values(df, numeric_strategy='drop', categorical_strategy='drop')
    ...
    def test_remove_outliers_zscore_method(self):
        ...
        # Create data with outliers
        df = pd.DataFrame({
            'values': [1, 2, 3, 4, 5, 50]  # 50 is 10+ std away
        })
    ...
    def test_remove_outliers_specific_columns(self):
        ...
        with patch.object(cleaner, 'remove_outliers', return_value=df.iloc[:-1]) as mock_remove:
    ...
    def test_clean_pipeline_full(self):
        ...
        assert len(cleaner.transformations_applied) >= 3  # At least standardize, remove_dups, handle_missing
    ...
    def test_clean_pipeline_selective(self):
        ...
        assert len(cleaner.transformations_applied) == 1
        assert cleaner.transformations_applied[0]['operation'] == 'standardize_column_names'
    ...
    def test_clean_pipeline_empty_dataframe(self):
        ...
        with patch.object(cleaner, 'standardize_column_names', return_value=df) as mock_standardize:
            result_df = cleaner.clean_pipeline(df)
    ...
    def test_outlier_removal_report(self):
        ...
        df = pd.DataFrame({
            'values': [1, 2, 3, 100]  # 100 is outlier
        })
    ...
    def test_data_types_preservation(self):
        ...
        result_df = cleaner.clean_pipeline(df, remove_dups=False, handle_missing=False, remove_outliers_flag=False)
    ...
    def test_logging_on_operations(self, mock_logger):
        ...
        df = pd.DataFrame({
            'col1': [1, 2, 3],
            'col2': [1, 2, 3]  # Duplicate column for removal
        })
