...
class RealtimePipeline:
    ...
    def _create_kafka_consumer(self, group_id: str = KAFKA_CONSUMER_GROUP) -> KafkaConsumer:
        """
    ...
    def load_from_kafka_batch(self, count: int = 1000, timeout_seconds: int = 60) -> pd.DataFrame:
        """
    ...
    def load_from_kafka_stream(self, 
                               callback: Any,
    ...
    def predict_batch(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        payload = {
            'transactions': df[features].to_dict(orient='records')
        }
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                df['fraud_probability'] = [r['fraud_probability'] for r in results['predictions']]
                ...
                logger.info(f"   Frauds detected: {fraud_count}/{len(df)} ({fraud_count/len(df)*100:.1f}%)")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if result['is_fraud']:
                    ...
                    logger.info(f"FRAUD DETECTED: txn {transaction['transaction_id']} "
    ...
    def execute_batch(self, count: int = 1000) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"      Frauds: {summary['fraud_detected']} ({summary['fraud_detected']/summary['consumed']*100:.1f}%)")
            ...
            elapsed = time.time() - start_time
            logger.error(f"Batch pipeline failed after {elapsed:.2f}s: {str(e)}", exc_info=True)
            return {
                'status': 'error',
                'message': str(e),
                'elapsed_seconds': elapsed
    ...
    def execute_stream(self, interval_seconds: int = 10) -> None:
        ...
        buffer = []
        last_process_time = [time.time()]  # Use list to allow modification in nested function
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Realtime Pipeline - Kafka → API → PostgreSQL')
    ...
    parser.add_argument('--kafka', default=KAFKA_BOOTSTRAP_SERVERS,
                       help='Kafka bootstrap servers')
    parser.add_argument('--topic', default=KAFKA_TOPIC,
                       help='Kafka topic')
    parser.add_argument('--api-url', default=API_URL,
                       help='API URL')
    parser.add_argument('--webapp-url', default=WEBAPP_URL,
                       help='Web app URL')
