...
class TransactionSimulator:
    ...
    def simulate_batch(self, count: int = 1000) -> Dict:
        ...
        logger.info(f"   Fraudulent: {summary['fraud_count']} ({summary['fraud_rate']:.1%})")
    ...
    def simulate_stream(self, 
        ...
        try:
            while True:
                ...
                # Check duration
                if duration_seconds and (time.time() - stream_start) >= duration_seconds:
            ...
            logger.info(f"    Fraudulent: {self.fraud_count} ({self.fraud_count/self.transaction_count:.1%})")
...
def main():
    ...
    parser.add_argument(
        '--topic', 
        default='fraud-detection-transactions', 
        help='Kafka topic'
    )
    parser.add_argument(
        '--fraud-rate', 
