...
class AnomalyDetector:
    ...
    def detect_null_anomalies(self, df: pd.DataFrame, threshold: float = 0.3) -> Dict:
        ...
        if result["has_anomalies"]:
            logger.warning(
                f"Columns with high null percentage (>{threshold*100}%): {anomalous_cols.to_dict()}"
            )
    ...
    def detect_distribution_anomalies(
        ...
        if result["has_anomalies"]:
            logger.warning(
                f"Columns with anomalous distributions: {list(anomalous_cols.keys())}"
            )
    ...
    def run_full_analysis(
        ...
        if expected_columns:
            report["missing_columns"] = self.detect_missing_columns(
                df, expected_columns
            )
