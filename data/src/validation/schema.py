...
class SchemaValidator:
    ...
    def validate_batch(
        self, df: pd.DataFrame, schema_type: str = "production"
    ) -> tuple[bool, dict]:
        """
        ...
        try:
            ...
            report["valid_rows"] = (
                len(df) if (fields_valid and types_valid and business_valid) else 0
            )
            report["invalid_rows"] = (
                0 if (fields_valid and types_valid and business_valid) else len(df)
            )
        ...
        for field, expected_type in self.field_types.items():
            ...
            try:
                if expected_type == str:
                    ...
                    if not all(
                        isinstance(x, (int, float, type(None))) for x in df[field]
                    ):
    ...
    def validate_business_rules(self, df: pd.DataFrame) -> tuple[bool, list]:
        ...
        if "currency" in df.columns:
            ...
            if len(invalid_currencies) > 0:
                errors.append(
                    f"Found {len(invalid_currencies)} invalid currency codes (expected 3-letter codes)"
                )
