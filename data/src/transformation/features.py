...
class FeatureEngineer(BaseFeatureEngineer):
    ...
    def create_temporal_features(
        self, df: pd.DataFrame, datetime_col: str = "time"
    ) -> pd.DataFrame:
        """
        ...
        df_features["is_weekend"] = (
            df_features[datetime_col].dt.dayofweek.isin([5, 6]).astype(int)
        )
        df_features["is_business_hours"] = (
            df_features["hour"].isin(range(9, 18)).astype(int)
        )
    ...
    def create_customer_features(
        ...
        df_features["customer_transaction_count"] = df_features.groupby(
            customer_id_col
        ).cumcount()

        # Customer amount statistics
        customer_stats = (
            df_features.groupby(customer_id_col)[amount_col]
        ...
        df_features["customer_std_amount"] = df_features["customer_std_amount"].fillna(
            0
        )
    ...
    def create_merchant_features(
        ...
        df_features["merchant_transaction_count"] = df_features.groupby(
            merchant_id_col
        ).cumcount()

        # Merchant amount statistics
        merchant_stats = (
            df_features.groupby(merchant_id_col)[amount_col]
        ...
        df_features["merchant_std_amount"] = df_features["merchant_std_amount"].fillna(
            0
        )
    ...
    def create_interaction_features(
        ...
        if (
            customer_col not in df_features.columns
            or merchant_col not in df_features.columns
        ):
        ...
        interaction_stats = (
            df_features.groupby([customer_col, merchant_col])[amount_col]
        ...
        df_features["customer_merchant_std_amount"] = df_features[
