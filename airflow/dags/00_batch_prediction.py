...
def validate_kafka_topic(**context):
    ...
    try:
        ...
        if total_messages < BATCH_SIZE:
            logger.warning(f" Only {total_messages} messages available, but batch size is {BATCH_SIZE}")
        
        # Push to XCom
        context['task_instance'].xcom_push(key='available_messages', value=total_messages)
...
def run_batch_prediction(**context):
    ...
    try:
        ...
        if result['status'] == 'success':
            ...
            context['task_instance'].xcom_push(key='fraud_detected', value=result['fraud_detected'])
            context['task_instance'].xcom_push(key='fraud_rate', value=result['fraud_rate'])
            context['task_instance'].xcom_push(key='saved', value=result['saved'])
            context['task_instance'].xcom_push(key='elapsed_seconds', value=result['elapsed_seconds'])
...
def generate_batch_report(**context):
    ...
    available = ti.xcom_pull(task_ids='validate_kafka_topic', key='available_messages') or 0
    ...
    fraud_detected = ti.xcom_pull(task_ids='run_batch_prediction', key='fraud_detected') or 0
    ...
    logger.info(f"   Data quality: {cleaned/consumed*100:.1f}%" if consumed > 0 else "   Data quality: N/A")
    logger.info("")
    ...
    logger.info(f"   Save rate: {saved/cleaned*100:.1f}%" if cleaned > 0 else "   Save rate: N/A")
    logger.info("")
    logger.info("⚡ PERFORMANCE")
    logger.info(f"   Total duration: {elapsed:.2f}s")
    logger.info(f"   Throughput: {consumed/elapsed:.1f} txn/s" if elapsed > 0 else "   Throughput: N/A")
    logger.info("=" * 80)
