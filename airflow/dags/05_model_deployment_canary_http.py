"""
DAG 05: Canary Deployment Pipeline (HTTP API Version)
======================================================
Progressive model deployment strategy: 5% → 25% → 100%

This DAG:
1. Compares Champion (Production) vs Challenger (Staging) from MLflow Registry
2. Deploys incrementally via HTTP API calls with monitoring at each stage
...
- Deployment is orchestrated via HTTP API calls (not DockerOperator)
"""
...
def decide_next_step(current_stage: str, **context):
    """
    ...
    # Get monitor result from previous task
    if current_stage == "5":
        monitor_task_id = "monitor_5_percent"
    elif current_stage == "25":
        monitor_task_id = "monitor_25_percent"
    else:
        return "deployment_complete"

    monitor_result = ti.xcom_pull(task_ids=monitor_task_id)
...
# Define the DAG
with DAG(
    dag_id="05_model_deployment_canary_http",
    default_args=default_args,
    description="Progressive canary deployment via HTTP API",
    ...
    # Task 6a: Deploy canary 25% via HTTP API
    deploy_canary_25_percent = SimpleHttpOperator(
    ...
    # Task 9a: Deploy 100% (promote to production) via HTTP API
    deploy_canary_100_percent = SimpleHttpOperator(
        task_id="deploy_canary_100_percent",
        http_conn_id="fraud_api_connection",
        endpoint="/admin/deployment/promote-to-production",
        method="POST",
        headers={
            "Content-Type": "application/json",
            "Authorization": "Bearer {{ var.value.API_ADMIN_TOKEN }}",
    ...
    # Task 9b: Rollback via HTTP API
    rollback_deployment = SimpleHttpOperator(
        task_id="rollback_deployment",
        http_conn_id="fraud_api_connection",
        endpoint="/admin/deployment/rollback-deployment",
        method="POST",
        headers={
            "Content-Type": "application/json",
            "Authorization": "Bearer {{ var.value.API_ADMIN_TOKEN }}",
