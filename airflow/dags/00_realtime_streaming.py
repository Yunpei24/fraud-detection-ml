...
def log_pipeline_metrics(**context):
    ...
    fraud_detected = ti.xcom_pull(task_ids='run_streaming_prediction', key='fraud_detected') or 0
    ...
    logger.info(f"   - Frauds detected: {fraud_detected} ({fraud_detected/consumed*100:.1f}%)" if consumed > 0 else "   - Frauds detected: 0")
