...
class FraudDetectionAlertOperator(BaseOperator):
    ...
    def _prepare_alert_payload(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'task_id': context.get('task_instance').task_id if 'task_instance' in context else 'unknown',
            'execution_date': context.get('execution_date').isoformat() if 'execution_date' in context else None,
    ...
    def _send_log_alert(self, alert_payload: Dict[str, Any]) -> Dict[str, str]:
        ...
        if severity == 'CRITICAL':
            ...
            self.logger.error(f"Details: {json.dumps(alert_payload['details'], indent=2)}")
        elif severity in ['WARNING', 'HIGH']:
            self.logger.warning(alert_msg)
            self.logger.warning(f"Details: {json.dumps(alert_payload['details'], indent=2)}")
        else:
            self.logger.info(alert_msg)
            self.logger.info(f"Details: {json.dumps(alert_payload['details'], indent=2)}")
    ...
    def _send_email_alert(
        ...
        try:
            ...
            to = context.get('dag').default_args.get('email', ['ml-alerts@frauddetection.com'])
